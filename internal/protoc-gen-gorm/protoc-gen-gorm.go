...
const (
	...
	clausePackage    = protogen.GoImportPath("gorm.io/gorm/clause")
...
type GormGenerator struct {
	...
	usedGoIdent        map[protogen.GoIdent]struct{}
	//m    *Message // 当时正在处理的message
}

func New(gen *protogen.Plugin, file *protogen.File, opts ...Option) *GormGenerator {
	...
	return &GormGenerator{
		...
		usedGoIdent:        make(map[protogen.GoIdent]struct{}),
	}
}
...
func (gg *GormGenerator) genMessage(m *Message) {
	...
	if _, ok := gg.usedGoIdent[m.GoIdent]; !ok && !setGorm(m) {
		return
	}
	...
	if defaultOptions.withGormOption {
...
func (gg *GormGenerator) changeGoType(m *Message, field *protogen.Field, goType string) string {
	...
	case protoreflect.MessageKind, protoreflect.GroupKind:
		if _, ok := specialMessageKinds[goType]; !ok {
			...
			gg.usedGoIdent[field.Message.GoIdent] = struct{}{}
		}
	}
		...
		if !field.Desc.IsMap() {
			...
			case protoreflect.MessageKind, protoreflect.GroupKind:
				if _, ok := specialMessageKinds[gi.oldGoType]; !ok {
					gg.g.P(firstLower(field.GoName), ", err := x.", field.GoName, ".", field.Message.GoIdent, structSuffix, "()")
					gg.g.P("if err != nil {")
					gg.g.P("return nil, err")
					gg.g.P("}")
					gg.g.P()
...
func (gg *GormGenerator) genModel2PbMethodFiled(m *Message, field *protogen.Field, gi goIdent) {
	if v := proto.GetExtension(field.Desc.Options(), option.E_Rules); v != nil {
		...
		if !field.Desc.IsMap() {
			...
			case protoreflect.MessageKind, protoreflect.GroupKind:
				if _, ok := specialMessageKinds[gi.oldGoType]; !ok {
					gg.g.P(firstLower(field.GoName), ", err := x.", field.GoName, ".", field.Message.GoIdent, "()")
					gg.g.P("if err != nil {")
					gg.g.P("return nil, err")
					gg.g.P("}")
					gg.g.P()
...
func (gg *GormGenerator) genNewType(m *Message) {
	...
	if has {
		...
		gg.g.P("func Value(data interface{}) (interface{}, error) {")
		...
		gg.g.P("func Scan(data interface{}, value interface{}) error {")
		gg.g.P("if value == nil {")
		gg.g.P("return nil")
		gg.g.P("}")
...
func (gg *GormGenerator) genNewTypeField(m *Message, gi goIdent) {
	...
	gg.g.P("}")
	gg.g.P()
	gg.g.P("func (x *", gi.newGoType, ") Scan(value interface{}) error {")
...
func (gg *GormGenerator) genIndexFunction(m *Message, field *protogen.Field) {
	...
	goType = gg.changeGoType(m, field, goType)
	gg.g.P("func ", m.GoIdent, structSuffix, field.GoName, "(x ", goType, ") Option {")

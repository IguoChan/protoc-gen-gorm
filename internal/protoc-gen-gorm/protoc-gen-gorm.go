...
func existExtension(msgs []*protogen.Message) bool {
	for _, m := range msgs {
		if proto.HasExtension(m.Desc.Options(), option.E_SetGorm) {
			return true
		}
	}
...
func (gg *GormGenerator) genMessage(m *Message) {
	...
	gg.g.P("}")
	gg.g.P()
	if proto.HasExtension(m.Desc.Options(), option.E_SetGorm) && !setGorm(m.parent) {
		gg.genTableNameMethod(m)
	}
	gg.genIndexFunction(m)
}
...
func (gg *GormGenerator) genIndexFunction(m *Message) {
	for _, field := range m.Fields {
		if proto.HasExtension(field.Desc.Options(), option.E_Rules) {
			gg.genPrimaryKeyFunction(m, field)
		}
	}
}

func (gg *GormGenerator) genPrimaryKeyFunction(m *Message, field *protogen.Field) {
	goType, pointer := gg.fieldGoType(field)
	if pointer {
		goType = "*" + goType
	}
